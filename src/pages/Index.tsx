      ...
      <section id="news" className="py-20 bg-secondary relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-20">
          ...
          <div className="space-y-8 text-lg">
            <p className="text-muted-foreground">Two papers accepted at the 2026 IEEE Conference on Decision and Control in Hawaii.</p>
            <div>
              <b><a href="https://scholar.google.com/citations?view_op=view_citation&hl=en&user=A1Nx-PQAAAAJ&citation_for_view=A1Nx-PQAAAAJ:KlAtU1dfN6UC" target="_blank" rel="noopener noreferrer" className="text-primary underline hover:text-primary/80">Integrated Routing and Intersection Control for Mixed Traffic</a></b> — with Pengbo Zhu and Andreas Malikopoulos
              <p className="mt-2 text-muted-foreground">Routing and intersection control of both CAVs and traffic signals in mixed traffic are usually handled as separate problems. In this work we tie them together. Especially, an upper layer optimization problem decides which route CAVs should take across the network, and a lower layer sets the traffic light phases and CAVs control inputs once they reach the proximity of an intersection. The joint optimization of the two layers offers really impressive results in travel time and network congestion! We tested it in SUMO on the Sioux Falls network, and running both layers together does considerably better than either one on its own.</p>
            </div>
            <div>
              <b><a href="https://scholar.google.com/citations?view_op=view_citation&hl=en&user=A1Nx-PQAAAAJ&citation_for_view=A1Nx-PQAAAAJ:kNdYIx-mwKoC" target="_blank" rel="noopener noreferrer" className="text-primary underline hover:text-primary/80">Closed-Form Characterization of Constrained Double-Integrator Optimal Control</a></b> — with Logan Beaver and Andreas Malikopoulos
              <p className="mt-2 text-muted-foreground">For energy-optimal control of a double integrator with speed and control limits, the optimal trajectory is a sequence of bang, unconstrained and coast arcs, and it is not always computationally trivial knowing where the switches fall in advance. We work out closed-form expressions for the switching times, by only knowing the initial conditions, in every admissible case, so they follow from closed loop expressions rather than a numerical search.</p>
            </div>
          </div>
        </div>
      </section>
